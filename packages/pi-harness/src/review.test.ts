...
function setup(choices: string[], editorReturns?: string) {
  ...
  const deps: Deps = { agentDir, harnessDir: join(agentDir, "harness"), sessionsDir: join(agentDir, "sessions"), dbPath: "" };
  ...
  writeProposal(deps.harnessDir, p);
...
describe("reviewOne", () => {
  test("accept applies and moves to applied", async () => {
    ...
    expect(listProposals(deps.harnessDir, "pending")).toEqual([]);
    expect(listProposals(deps.harnessDir, "accepted")).toHaveLength(1);
  ...
  test("edit then accept uses edited text", async () => {
    ...
    expect(listProposals(deps.harnessDir, "accepted")[0].change).toMatchObject({ replace: "- Never commit\n- Use bun test" });
  });
  ...
  test("reject moves without touching files", async () => {
    ...
    expect(listProposals(deps.harnessDir, "rejected")).toHaveLength(1);
  });

  test("later leaves it pending", async () => {
    const { deps, p, ctx } = setup(["Later"]);
    await reviewOne(ctx, deps, p);
    expect(listProposals(deps.harnessDir, "pending")).toHaveLength(1);
  });
});
